...
void renderMesh(Mesh *mesh) {
	...
	Matrix T = TranslationMatrix(10, 0, 0);
	Matrix R_x = Rx(0);
	Matrix R_y = Ry(90);
	Matrix R_z = Rz(0);
	Matrix S = ScaleMatrix(1, 1, 1);

	Matrix W = MatMatMul(T, MatMatMul(R_x, MatMatMul(R_y, MatMatMul(R_z, S))));
	M = MatMatMul(V, W);

	// Pass the viewing transform to the shader

	//GLint loc_M = glGetUniformLocation(shprg, "M");   //It does not work
...
int main(int argc, char **argv) {
	...
	int count, model, px, py, pz, rx, ry, rz, sx, sy, sz;
